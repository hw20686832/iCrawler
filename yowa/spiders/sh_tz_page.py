...
class sh_tz_Spider(CrawlSpider):
    ...
    def parse_525j(self,response):
        '''我爱我家知道网'''
        hxs = HtmlXPathSelector(response)
        hxs_page = hxs.select("//p[@class='page']//a")
        for p in hxs_page:
            if p.select('./text()').extract()[0] == u'[下一页]':
                ...
                yield Request(url = url, meta = response.meta, callback = self.parse_525j)
    ...
    def parse_tumanduo(self,response):
        '''装修图满多'''
        hxs = HtmlXPathSelector(response)
        hxs_page = hxs.select("//li[@class='nextpage']/a/@href")
        url = hxs_page.extract()[0]
        if not url.startswith('http'):
            base_url = get_base_url(response)
            url = urljoin_rfc(base_url, url)
    ...
    def parse_zhaogewo(self,response):
        '''找个窝'''
        hxs = HtmlXPathSelector(response)
        hxs_page = hxs.select("//div[@class='pages-nav']//a")
        for p in hxs_page:
            if p.select('./text()').extract()[0] == u'下一页':
                ...
                yield Request(url = url, meta = response.meta, callback = self.parse_zhaogewo)
    ...
    def parse_39(self,response):
        '''39健康问答'''
        hxs = HtmlXPathSelector(response)
        hxs_page = hxs.select("//a[@class='next']/@href")
        url = hxs_page.extract()[0]
        if not url.startswith('http'):
            base_url = get_base_url(response)
            url = urljoin_rfc(base_url, url)
    ...
    def parse_120ask(self,response):
        '''有问必答网'''
        hxs = HtmlXPathSelector(response)
        hxs_page = hxs.select("//div[@class='fanye01']//a")
        for p in hxs_page:
            if p.select('./text()').extract()[0] == u'下一页':
                ...
                yield Request(url = url, meta = response.meta, callback = self.parse_120ask)
    ...
    def parse_movshow(self, response):
        '''猫咪有约'''
        hxs = HtmlXPathSelector(response)
        hxs_page = hxs.select("//a[@class='nxt']/@href")
        url = hxs_page.extract()[0]
        if not url.startswith('http'):
            base_url = get_base_url(response)
            url = urljoin_rfc(base_url, url)

        yield Request(url = url, meta = response.meta, callback = self.parse_movshow)
        
        hxs_a = hxs.select("//tbody[contains(@id,'normalthread')]//th[@class = 'new']/a[@class = 'xst']/@href").extract()
        hxs_a.extend(hxs.select("//tbody[contains(@id,'normalthread')]//th[@class = 'common']/a[@class = 'xst']/@href").extract())
        for a in hxs_a:
            if not a.startswith('http'):
                base_url = get_base_url(response)
                a = urljoin_rfc(base_url, a)
    ...
    def parse_goumin(self, response):
        '''狗民网'''
        hxs = HtmlXPathSelector(response)
        hxs_page = hxs.select("//div[@id='pagenav']//a")
        for p in hxs_page:
            if p.select('./text()').extract()[0] == u'下一页':
                ...
                yield Request(url = url, meta = response.meta, callback = self.parse_goumin)
    ...
    def parse_xcar(self, response):
        '''爱卡汽车网'''
        hxs = HtmlXPathSelector(response)
        hxs_page = hxs.select("//a[@class='xyiye']/@href")
        url = hxs_page.extract()[0]
        if not url.startswith('http'):
            base_url = get_base_url(response)
            url = urljoin_rfc(base_url, url)

        yield Request(url = url, meta = response.meta, callback = self.parse_xcar)
