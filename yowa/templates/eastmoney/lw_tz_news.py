...
class Parser(Base):
    name = 'eastmoney_tz_news'

    def extract(self):
        self.html = re.sub('<!--.*?-->', '', self.html)
        doc = PyQuery(self.html)
        content_node = doc('.bk4').eq(0)
        ...
        item['title'] = self.title = content_node('.biaoti1').text()
        item['content'] = self.content = content_node('.zw').__unicode__()
        
        release_time=content_node('.zuozhe1').eq(0).text()
...
#        item['release_switch_time'] = self.release_switch_time = time.mktime(time.strptime(release_time[0],u'%Y-%m-%d %H:%M:%S'))
        ...
        author=content_node('.zuozhe1').eq(0).text()
        ob=re.compile(u' .* 2')
        author=ob.findall(author)
        author = author[0].replace(u'2','')
