...
class Parser(Base):
    ...
    def extract(self):
        ...
        content_node.remove('.a_pr')
        if not content_node:
            content_node=doc('.t_msgfontfix').eq(0)
        if not content_node:
            content_node = doc('div#BSHARE_HTML')
        ...
        if not release_time:
            release_time=doc('.authi')('em')('span').eq(0)
...
#        item['release_switch_time'] = self.release_switch_time = time.mktime(time.strptime(release_time,u'%Y-%m-%d %H:%M:%S'))
        item['source'] = u"口袋巴士"
        item['author']  = self.author = doc('.user_id')('a').eq(0).text()
        if not item['author']:
            item['author'] = self.author = doc('.authi')('a').eq(0).text()
        if not item['author']:
            item['author'] = self.author =doc('.postinfo')('a').eq(0).text()
        item['pic_url'] = ''
