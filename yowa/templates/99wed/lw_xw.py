...
class Parser(Base):
    ...
    def extract(self):
        ...
        content_node = doc('#news_content_picjieshao')
        content_node.remove('span[class = "news_content_pi_tishi"]')
        ...
        release_time=doc('#zuoze').text()
        ob=re.compile(u'20\d\d-\d*-\d*')
        release_time=ob.findall(release_time)
                
        item['title'] = self.title = doc('div.news_content_title').text()
