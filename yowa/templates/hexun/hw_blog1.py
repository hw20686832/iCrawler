...
class Parser(Base):
    ...
    def extract(self):
        ...
        content_node = doc('div#BlogArticleDetail')
        ...
        self.title = doc('span.ArticleTitleText').text()
        item['title'] = self.title
        item['content'] = self.content = content_node.__unicode__()
        release_time=doc('div.ArticleTitle').text()
...
#        item['release_switch_time'] = self.release_switch_time = time.mktime(time.strptime(release_time,u'%Y��%m��%d��%H:%M'))
        item['source'] = u"和讯博客"
        item['author'] = doc('div.site_description_A').text()
        item['pic_url'] = ''
