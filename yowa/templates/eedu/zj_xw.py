...
class Parser(Base):
    ...
    def extract(self):
        ...
        content_node = doc('div#textbody')
        content_node.remove('iframe')
        content_node.remove('div#ckepop')
        content_node.remove('script')
        content_node.remove(u'a[title = "环境生态网更多精彩资讯"]')
        content_node.remove('span').find('li')
        ...
        release_time=doc('div.from_info').eq(1).text()
        r = re.compile(u"(20\d\d-\d?-\d\d)")
